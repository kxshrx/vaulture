...
def check_rate_limit(user_id: int, file_path: str) -> bool:
    ...
    keys_to_remove = [k for k in download_attempts.keys() if int(k.split(':')[1]) < (current_time // 60) - 5]
...
async def serve_file(
    ...
    if not check_rate_limit(current_user.id, file_path):
        logger.warning(f"Rate limit exceeded: user_id={current_user.id}, ip={client_ip}, file={file_path}")
        raise HTTPException(status_code=429, detail="Too many download attempts. Please try again later.")
    
    # Verify the token first (time-based security)
    if not verify_file_token(file_path, token, expires):
        logger.warning(f"Invalid/expired token: user_id={current_user.id}, ip={client_ip}, file={file_path}, token={token[:8]}...")
    ...
    if not has_access:
        ...
        logger.warning(f"Unauthorized file access attempt: user_id={current_user.id}, ip={client_ip}, product_id={product.id}, file={file_path}")
        raise HTTPException(
            status_code=403, 
    ...
    logger.info(f"File accessed: user_id={current_user.id}, ip={client_ip}, product_id={product.id}, file={file_path}")
