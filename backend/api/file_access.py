...
def access_file(
    ...
    if not product:
        logger.warning(
            f"Product not found: product_id={product_id}, user_id={current_user.id}"
        )
    ...
    if not is_creator_owner:
        ...
        if not purchase:
            logger.warning(
                f"Unauthorized access attempt: user_id={current_user.id}, product_id={product_id}"
            )
            raise HTTPException(
                status_code=403,
    ...
    try:
        ...
        logger.info(
            f"File access granted: user_id={current_user.id}, product_id={product_id}, access_type={access_type}"
        )
        ...
        logger.error(
            f"File access error: user_id={current_user.id}, product_id={product_id}, error={str(e)}"
        )
