...
class ImageUploadSchema(BaseModel):
    ...
    filename: str = Field(..., description="Original filename")
    content_type: Optional[str] = Field(None, description="MIME content type")
    size: int = Field(..., ge=1, le=50*1024*1024, description="Image size in bytes (max 50MB)")
    ...
    def validate_image_filename(cls, v):
        ...
        if extension not in allowed_image_types:
            raise ValueError(f'Image type "{extension}" not allowed. Allowed: {", ".join(allowed_image_types)}')
