...
def get_user_profile(db: Session, user_id: int) -> UserProfileResponse:
    ...
    if user.is_creator:
        ...
        total_products = (
            db.query(Product)
            .filter(Product.creator_id == user_id, Product.is_active == True)
            .count()
        )
        ...
        total_revenue = (
            db.query(func.sum(Product.price))
...
def change_user_password(
    db: Session, user_id: int, password_data: PasswordChangeRequest
) -> dict:
...
def search_users(
    db: Session, query: str, user_type: Optional[str] = None, limit: int = 20
):
    ...
    return [
        PublicProfileResponse(
            ...
            total_products=(
                db.query(Product)
                .filter(Product.creator_id == user.id, Product.is_active == True)
                .count()
                if user.is_creator
                else None
            ),
